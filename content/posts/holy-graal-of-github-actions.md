...
---
  ...
  [native_yaml]: https://github.com/sbt/sbtn-dist/blob/56ec2e8d7bcbe5b7fbc9325d99639d66b77b338c/.github/workflows/native.yml
  ...
  [github-blog-2025-01-16]: https://github.blog/changelog/2025-01-16-linux-arm64-hosted-runners-now-available-for-free-in-public-repositories-public-preview/
...
GraalVM is available for macOS and Linux on x86-64 and ARM/AArch64, and for Windows on x86_64. As of [2024-01-30][github-blog-2024-01-30], we have the ability to produce native image on all supported OSes and architectures, and as of [2025-01-16][github-blog-2025-01-16] without QEMU emulation:

| OS      | x86-64      | ARM/AArch64 |
|---------|------------:|------------:|
| macOS   | macOS-13    | macOS-14    |
| Linux   | ubuntu-20.04| ubuntu-22.04-arm|
...
1. Build x86-64 binary on macOS-13, and upload it to upload-artifact space.
2. Build ARM binary on macOS-14, and upload it to upload-artifact space.
3. Use download-artifact to download both files on another macOS-13 job, and call `lipo` to create a universal binary.
...
## appendix A: building an ARM Linux binary via QEMU

In 2024 when I first wrote this, GitHub Actions [did not support ARM Linux](https://github.com/actions/runner-images/blob/df722a3cf81bbe65c3b964c9b4830c85451542fc/README.md#available-images).
However, we could still build an ARM Linux binary using [uraimo/run-on-arch-action](https://github.com/uraimo/run-on-arch-action), which runs [QEMU](https://www.qemu.org/), a machine emulator that can emulate other CPUs.
...
The fact that this was possible at all was interesting, but it did come with a cost of time. If the normal Linux build takes 3 minutes, this would take 30 minutes.
