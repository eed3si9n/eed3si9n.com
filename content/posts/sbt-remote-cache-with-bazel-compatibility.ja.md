...
sbt 2.x のキャッシュに戻ると、これまでの所 `compile` タスクを例に汎用的な機構を作るための基盤を入れ替えることに集中してきたので、リモート・キャッシュと言えどまだ「分散」の部分には手を出していなかった。本稿ではこれを見ていく。実装の提案は [sbt/sbt#7525](https://github.com/sbt/sbt/pull/7525) にある。
